...
type LoadBalancerItem struct {
	LoadBanlance balance.LoadBalance
	ServiceName  string
}
...
func (lbr *LoadBalancer) GetLoadBalancer(service *ServiceDetail) (balance.LoadBalance, error) {
	...
	mConf, err := balance.NewLoadBalanceCheckConf(fmt.Sprintf("%s%s", schema, "%s"), ipConf)
	if err != nil {
		return nil, err
	}
	lb := balance.LoadBanlanceFactorWithConf(balance.LbType(service.LoadBalance.RoundType), mConf)
